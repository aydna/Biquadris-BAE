...
NormalBoard::Board() {}; // virtual; to be overrided by blind_block

NormalBoard::Board(int seed, std::string scriptfile, int startLevel):
    level{startLevel},
    seed{seed},
    file{scriptfile}{
...
std::vector<std::pair<int,int>> NormalBoard::getBoardPixels(int offBack){ 
...
bool NormalBoard::validMove(std::vector<std::pair<int,int>> pixels){
...
void NormalBoard::swapBlock(std::string type){
    blocks.pop_back();
...
//level up by times
void NormalBoard::levelUp(int times){
    level += times; 
    if (level > 4) level = 4;
    if (level == 0) currLevel = std::make_unique<Level0>(seed, file);
    else if (level == 1) currLevel = std::make_unique<Level1>(seed, file);
...
//level down by times
void NormalBoard::levelDown(int times){
    level -= times; 
    if (level < 0) level = 0;
    if (level == 0) currLevel = std::make_unique<Level0>(seed, file);
    else if (level == 1) currLevel = std::make_unique<Level1>(seed, file);
...
void NormalBoard::moveRight(int times){
...
void NormalBoard::moveLeft(int times){
    std::vector<std::pair<int,int>> pixels = blocks.back()->getPixels();
    for (int i = 0; i < times; ++i){
        std::vector<std::pair<int,int>> testPixels = pixels;
...
void NormalBoard::moveDown(int times){
    std::vector<std::pair<int,int>> pixels = blocks.back()->getPixels();
    for (int i = 0; i < times; ++i){
        std::vector<std::pair<int,int>> testPixels = pixels;
...
void NormalBoard::rotateCW(int times){
    std::vector<std::pair<int,int>> pixels = blocks.back()->getPixels();
...
void NormalBoard::rotateCCW(int times){
    rotateCW((times % 4)* 3);       //works <3
}

//moves the block down as far as it go and then removes the rows that can be removed
int NormalBoard::drop(){
    moveDown(18); 
    return removeRow();
}

int NormalBoard::getLevel(){ return level; }

int NormalBoard::getScore(){ return score; }
...
void NormalBoard::checkGameOver(std::vector<std::pair<int,int>> pixels){
...
std::unique_ptr<Board>& NormalBoard::clearSpecial() {
    return nullptr;
}

bool NormalBoard::gameOver(){ return gameOverSwitch; }


void NormalBoard::blindPlayer() {
